# 💀 TABBO REPORT TOOL

A powerful Python-based reporting automation tool designed for Termux / Linux environments.

This tool demonstrates automation concepts such as threading, request handling, and terminal interface design.

--------------------------------------------------

⭐ FEATURES

- Fast Multi Thread Engine
- Colored Terminal Interface
- Live Statistics Counter
- Random Email Generator
- Batch Processing System
- Easy Command Line Usage

--------------------------------------------------

📦 REQUIREMENTS

- Python 3
- Termux / Linux
- Internet Connection

Install required libraries:

```
pip install requests
```

--------------------------------------------------

🚀 INSTALLATION (TERMUX)

1️⃣ Update packages

```
pkg update -y
```

2️⃣ Install dependencies

```
pkg install git python -y
```

3️⃣ Clone repository

```
git clone https://github.com/tabrej12787/tabbo-info-tool.git
```

4️⃣ Enter project folder

```
cd tabbo-info-tool
```

5️⃣ Install Python libraries

```
pip install requests
```

6️⃣ Run the tool

```
python report.py
```

--------------------------------------------------

📖 GUIDE

1. Run the tool using the command:

```
python report.py
```

2. Enter the required target information.

3. Choose the number of threads.

4. The tool will start processing and display live statistics.

5. Press **CTRL + C** anytime to stop the process.

--------------------------------------------------

📊 TOOL INFORMATION

Language : Python  
Platform : Termux / Linux  
Interface : Terminal CLI  

--------------------------------------------------

📝 CHANGELOG

Version 1.0
- Initial release
- Multi-thread system added
- Colored terminal interface

Version 1.1
- Improved speed
- Added live statistics counter
- Minor bug fixes

--------------------------------------------------

⚠️ DISCLAIMER

This project is created for educational and research purposes only.

The developer is not responsible for any misuse or illegal activities performed using this tool.

--------------------------------------------------

👨‍💻 DEVELOPER

TABBO

Telegram : https://t.me/tabboinfo1

--------------------------------------------------

⭐ If you found this project useful, consider giving it a star on GitHub.
